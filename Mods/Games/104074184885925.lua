...
local RebirthEnabled, CashEnabled = false, false
...
local function AutoCash()
	if CashEnabled then
     table.clear(CashActives)

    
     Plot.ChildAdded:Connect(function(CashActives)
          
    end
     workspace.Plot_KopiPahitGamer.Floor1
     workspace.Plot_KopiPahitGamer.Floor2
     workspace.Plot_KopiPahitGamer.Floor1.Slots.Slot6.Button.Touch
...
-- Cached Event Path: ReplicatedStorage.Events.RequestRebirth
local Event = game:GetService("ReplicatedStorage").Events.RequestRebirth
Event:FireServer()
game:GetService("Players").LocalPlayer.PlayerGui.GUI.Frames.Rebirth.Bar.Progress.Size.X.Scale=1

workspace.ItemSpawners.Squishy
workspace.ItemSpawners.Squishy.DroppedItem.SNOWFLAKE.PickupPrompt
